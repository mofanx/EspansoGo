...
| 传输方式 | 触发机制 | 方向 | 适用场景 |
|---------|---------|------|---------|
| **CloudFolder** | WorkManager 定时 + 文件监控 | 双向 | 普通用户，已有云存储 |
| **GitRepo** | WorkManager 定时 + 手动 | 双向 | 开发者，需要版本历史 |
| **LocalNetwork** | 按需（同局域网时） | 双向 | 高频同步，无云依赖 |
| **ManualFile** | 用户手动触发 | 单次 | 偶尔同步，无云服务 |

#### 决策 3：后台同步 + 前台通知

**Android 端**：
- 使用 `WorkManager` 周期性任务（每 15 分钟）检查同步源
- 文件变化时通过 `WeakReferenceMessenger` 通知 AccessibilityService 更新 `dict`
- 同步状态显示在 UI 顶部（上次同步时间、状态图标）

**桌面端**：
- 无需额外开发 — espanso 的 `auto_restart` 已内置文件监控
- 用户只需将 espanso 的 `match/` 目录放入同步文件夹

---

## 传输方案详细设计

### 方案 A：CloudFolder（云文件夹同步）— 推荐默认

**原理**：指定一个云存储同步的文件夹作为 espanso match 目录的镜像，Expandroid 和桌面都读写这个文件夹。

**桌面端设置**（一次性）：
```bash
# 方法 1：直接将 match 目录放入云同步文件夹
ln -s ~/Google\ Drive/espanso-match/ ~/.config/espanso/match

# 方法 2：或反过来，让云同步工具同步 espanso 的 match 目录
# 在 Google Drive / Dropbox 客户端中添加 ~/.config/espanso/match/
```
...
桌面修改 → 保存到 match/*.yml → 云客户端自动上传
                                    ↓
Android WorkManager 检测到文件变化 → 解析 YAML → 更新 dict
                                    ↓
...
- 支持所有主流云服务（Google Drive、Dropbox、OneDrive、Nextcloud）
- 不增加 APK 体积（SAF 是 Android 原生）

**缺点**：
- 依赖云服务和网络
- 非实时（WorkManager 最短周期 15 分钟）
- 需要桌面端安装对应云客户端

**改动量**：中等 — SyncManager + YamlWorkspace + WorkManager + 设置向导 UI

---

### 方案 B：GitRepo（Git 仓库同步）— 开发者首选
...
### 主方案：CloudFolder + Hub 客户端

| 需求 | 方案 | 理由 |
|------|------|------|
| 个人 matches 双向同步 | CloudFolder | 改动适中，用户门槛低，利用现有云存储 |
...
│  3. 云客户端自动上传到云存储                                   │
│  4. Android WorkManager 检测到变化（≤15分钟）                 │
...
│  2. 保存时同步触发：序列化为 YAML → 写入云文件夹               │
│  3. 云客户端自动上传                                          │
│  4. 桌面云客户端同步到本地                                    │
│  5. espanso auto_restart 检测变化 → 自动重载                  │
└──────────────────────────────────────────────────────────────┘
...
SyncManager
├── 配置
│   ├── SyncMethod (Cloud / Git / Local / Manual)
│   ├── SyncUri (SAF URI / Git URL / HTTP URL)
│   ├── SyncInterval (15min / 30min / 1hour / Manual only)
│   └── LastSyncTime
├── Pull()     → 从同步源拉取 → YamlWorkspace.ReadFromFolder → 更新 dict
├── Push()     → YamlWorkspace.WriteToFolder → 写入同步源
├── CheckChanges() → 对比文件时间戳，检测是否有变化
...
WizardStep 3b: "输入 Git 仓库信息"
  └─ URL + PAT → clone → Step 4

WizardStep 3c: "扫描桌面二维码"
  └─ 相机扫码 → 连接 → Step 4

WizardStep 3d: "选择 YAML 配置文件"
  └─ 文件选择器 → 导入 → 完成
...
| `SyncManager` 服务 | 新建 `Services/SyncManager.cs` | 高 |
| SAF 持久化 URI 管理 | `AppSettings.cs` + 新建 `Services/SafManager.cs` | 高 |
...
| 实时同步（< 15 秒） | WorkManager 最短周期 15 分钟，实时需前台服务（耗电） |
